...
using SystemCommands = RxFramework.WPF.FluentControls.Windowing.SystemCommands;
...
public partial class WindowChromeBehavior
{
    ...
    private WINDOWPOS _lastWindowpos;

#pragma warning disable 414
    private bool _isDragging;
#pragma warning restore 414

    private NonClientControlManager? _nonClientControlManager;
    ...
    {
        return WindowProc(hwnd, msg, (nuint)wparam.ToInt64(), lparam, ref handled);
    }
    ...
    private IntPtr WindowProc(IntPtr hwnd, int msg, nuint wParam, nint lParam, ref bool handled)
    {
        ...
        switch (message)
        {
            case WM.NCUAHDRAWCAPTION:
                return _HandleNCUAHDRAWCAPTION(message, wParam, lParam, out handled);
            case WM.SETTEXT:
            case WM.SETICON:
                return _HandleSETICONOrSETTEXT(message, wParam, lParam, out handled);
            case WM.SYSCOMMAND:
                return _HandleSYSCOMMAND(message, wParam, lParam, out handled);
            case WM.NCACTIVATE:
                return _HandleNCACTIVATE(message, wParam, lParam, out handled);
            case WM.NCCALCSIZE:
                return _HandleNCCALCSIZE(message, wParam, lParam, out handled);
            case WM.NCHITTEST:
                return _HandleNCHITTEST(message, wParam, lParam, out handled);
            case WM.NCPAINT:
                return _HandleNCPAINT(message, wParam, lParam, out handled);
            case WM.NCRBUTTONUP:
                return _HandleNCRBUTTONUP(message, wParam, lParam, out handled);
            case WM.SIZE:
                return _HandleSIZE(message, wParam, lParam, out handled);
            case WM.WINDOWPOSCHANGING:
                return _HandleWINDOWPOSCHANGING(message, wParam, lParam, out handled);
            case WM.WINDOWPOSCHANGED:
                return _HandleWINDOWPOSCHANGED(message, wParam, lParam, out handled);
            case WM.GETMINMAXINFO:
                return _HandleGETMINMAXINFO(message, wParam, lParam, out handled);
            ...
            case WM.EXITSIZEMOVE:
                ...
                break;
            case WM.MOVE:
                return _HandleMOVEForRealSize(message, wParam, lParam, out handled);
            case WM.STYLECHANGING:
                return _HandleStyleChanging(message, wParam, lParam, out handled);
            case WM.DESTROY:
                return _HandleDestroy(message, wParam, lParam, out handled);

            case WM.NCMOUSEMOVE:
                return _HandleNCMOUSEMOVE(message, wParam, lParam, out handled);
            case WM.NCLBUTTONDOWN:
                return _HandleNCLBUTTONDOWN(message, wParam, lParam, out handled);
            case WM.NCLBUTTONUP:
                return _HandleNCLBUTTONUP(message, wParam, lParam, out handled);
            case WM.NCRBUTTONDOWN:
            case WM.NCRBUTTONDBLCLK:
                return _HandleNCRBUTTONMessages(message, wParam, lParam, out handled);
            case WM.MOUSEMOVE:
                return _HandleMOUSEMOVE(message, wParam, lParam, out handled);
            case WM.NCMOUSELEAVE:
                return _HandleNCMOUSELEAVE(message, wParam, lParam, out handled);
            case WM.MOUSELEAVE:
                return _HandleMOUSELEAVE(message, wParam, lParam, out handled);
        }
    ...
    private IntPtr _HandleNCUAHDRAWCAPTION(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
            ...
            && _GetHwndState() == WindowState.Minimized)
        {
            ...
            using (new SuppressRedrawScope(_windowHandle))
            {
                handled = true;
                return PInvoke.DefWindowProc(_windowHandle, (uint)uMsg, wParam, lParam);
            }
        }
    ...
    private IntPtr _HandleNCACTIVATE(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
        ...
        var lRet = PInvoke.DefWindowProc(_windowHandle, (uint)WM.NCACTIVATE, wParam, new IntPtr(-1));
        ...
        if (IsNCActive == false)
        {
            _nonClientControlManager?.ClearTrackedControl();
        }
    ...
    private IntPtr _HandleNCCALCSIZE(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
        ...
        if (hwndState == WindowState.Maximized)
        {
            ...
            var monitor = MonitorHelper.MonitorFromWindowPosOrWindow(_lastWindowpos, _windowHandle);
            var monitorInfo = PInvoke.GetMonitorInfo(monitor);
            //System.Diagnostics.Trace.WriteLine(monitorInfo.rcWork);

            var monitorRect = HideTaskbarOnMaximize
            ...
            if (HideTaskbarOnMaximize == false
        ...
        else if (PInvoke.GetWindowStyle(_windowHandle).HasFlag(WINDOW_STYLE.WS_CAPTION))
        {
            var rcBefore = Marshal.PtrToStructure<RECT>(lParam);
            PInvoke.DefWindowProc(_windowHandle, (uint)uMsg, wParam, lParam);
            var rc = Marshal.PtrToStructure<RECT>(lParam);
    ...
    {
        var isRightToLeft = AssociatedObject.FlowDirection == FlowDirection.RightToLeft;
    ...
    private IntPtr _HandleNCHITTEST(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
        ...
        var hitTestResult = GetHitTestResult(lParam);
    ...
    {
        if (NonClientControlManager.GetControlUnderMouse(AssociatedObject, lParam, out var htFromNcControlManager) is not null
        ...
        var preventResize = _GetHwndState() == WindowState.Maximized || AssociatedObject.ResizeMode is ResizeMode.NoResize;
        var htFromTestNca = preventResize
            ? HT.CLIENT
            : _HitTestNca(DpiHelper.DeviceRectToLogical(windowRect, dpi.DpiScaleX, dpi.DpiScaleY),
        ...
        {
            var mousePosWindow = AssociatedObject.PointFromScreen(mousePosScreen);
            ...
            if (inputElement is not null)
            {
                ...
                if (AssociatedObject.ResizeMode == ResizeMode.CanResizeWithGrip)
                {
                    var direction = WindowChrome.GetResizeGripDirection(inputElement);
                    if (direction != ResizeGripDirection.None)
                    {
                        return _GetHTFromResizeGripDirection(direction);
                    }
                }
            }
        }
    ...
    private IntPtr _HandleNCRBUTTONUP(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
        ...
        if (hitTest is HT.CAPTION or HT.MINBUTTON or HT.MAXBUTTON or HT.CLOSE)
        {
            ...
            SystemCommands.ShowSystemMenuPhysicalCoordinates(AssociatedObject, Utility.GetPoint(lParam));
        }
    ...
    private IntPtr _HandleSIZE(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
        ...
        // if (NativeMethods.DwmGetWindowAttribute(_windowHandle, DWMWINDOWATTRIBUTE.WINDOW_CORNER_RADIUS, out var radius, sizeof(int)) == HRESULT.S_OK)
        // {
        //     DWMCornerRadius = radius;
        // }
        ...
        // Still let the default WndProc handle 
        handled = false;
        return IntPtr.Zero;
    }
    ...
    private IntPtr _HandleWINDOWPOSCHANGING(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
            ...
            && windowpos.cy > 0)
        {
            ...
            var finalRect = _isDragging
                ? rect
                : MonitorHelper.GetOnScreenPosition(rect, _windowHandle, HideTaskbarOnMaximize);
    ...
    private IntPtr _HandleGETMINMAXINFO(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
         ...
         */
        if (HideTaskbarOnMaximize
            && PInvoke.IsZoomed(_windowHandle))
        {
            var mmi = Marshal.PtrToStructure<MINMAXINFO>(lParam);

            var monitor = PInvoke.MonitorFromWindow(_windowHandle, MONITOR_FROM_FLAGS.MONITOR_DEFAULTTONEAREST);
    ...
    private IntPtr _HandleMOVEForRealSize(WM uMsg, nuint wParam, nint lParam, out bool handled)
    {
        ...
        {
            var monitorFromWindow = PInvoke.MonitorFromWindow(_windowHandle, MONITOR_FROM_FLAGS.MONITOR_DEFAULTTONEAREST);
            if (monitorFromWindow != IntPtr.Zero)
            {
                var ignoreTaskBar = HideTaskbarOnMaximize;
                 ...
                 * 
                 */
                PInvoke.SetWindowPos(_windowHandle, default, rcMonitorArea.left, rcMonitorArea.top, rcMonitorArea.GetWidth(), rcMonitorArea.GetHeight(), SET_WINDOW_POS_FLAGS.SWP_ASYNCWINDOWPOS | SET_WINDOW_POS_FLAGS.SWP_FRAMECHANGED | SET_WINDOW_POS_FLAGS.SWP_NOCOPYBITS);
            }
        }
    ...
    {
        if (isTrackingMouseEvents == false)
        {
            ...
            isTrackingMouseEvents = true;
        }

        handled = _nonClientControlManager?.HoverTrackedControl(lParam) == true;
    ...
    {
        handled = _nonClientControlManager?.PressTrackedControl(lParam) == true;
    ...
    {
        handled = _nonClientControlManager?.ClickTrackedControl(lParam) == true;
    ...
    {
        handled = true;
        PInvoke.RaiseNonClientMouseMessageAsClient(_windowHandle, uMsg, wParam, lParam);
    ...
    {
        var dwStyle = PInvoke.GetWindowStyle(_windowHandle);
        ...
        PInvoke.SetWindowStyle(_windowHandle, dwNewStyle);
        return true;
    }
    ...
    {
        var wpl = PInvoke.GetWindowPlacement(_windowHandle);
        switch (wpl.showCmd)
        {
    ...
    private Rect _GetWindowRect()
    {
        ...
        var windowPosition = PInvoke.GetWindowRect(_windowHandle);
    ...
    private void _UpdateSystemMenu(WindowState? assumeState)
    {
            ...
            || _lastMenuState != state)
        {
            ...
            var menuHandle = PInvoke.GetSystemMenu(_windowHandle, false);
    ...
    private void UpdateWindowStyle()
    {
        if (HideTaskbarOnMaximize
            && _GetHwndState() == WindowState.Maximized)
        {
            _ModifyStyle(WINDOW_STYLE.WS_CAPTION, 0);
        }
        else
        {
            //_ModifyStyle(WINDOW_STYLE.WS_SYSMENU, WINDOW_STYLE.WS_CAPTION);
            _ModifyStyle(0, WINDOW_STYLE.WS_CAPTION); // todo: mica
        }
    }
    ...
    private HT _HitTestNca(Rect windowRect, Point mousePosition)
    {
        ...
        if (uRow != 1
            && uCol == 1)
        {
            uCol = GetHTColumn(windowRect, mousePosition, _cornerGripThickness);
        }
        else if (uCol != 1
                 && uRow == 1)
        {
            uRow = GetHTRow(windowRect, mousePosition, _cornerGripThickness);
        }
